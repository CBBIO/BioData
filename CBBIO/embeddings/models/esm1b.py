...
class Esm1bModelAdapter(ModelAdapter):
    ...
    def __init__(
        ...
        model: Any,
        *,
        available_layer_count: int | None = 33,
        normalize_hf_hidden_states: bool = True,
    ) -> None:
        self.model = model
        self.available_layer_count = available_layer_count
        self.normalize_hf_hidden_states = bool(normalize_hf_hidden_states)
        ...
        *,
        ...
        super().__init__(
            ...
            tokenizer=Esm1bTokenizerAdapter(
                ...
                int(padding_idx) if isinstance(padding_idx, int) else None,
                tokenizer=resolved_converter if resolved_alphabet is None else None,
                device=device,
            ),
            model=Esm1bModelAdapter(
                resolved_model,
                available_layer_count=33,
                normalize_hf_hidden_states=normalize_hf_hidden_states,
            ),
        ...
        parameters: Dict[str, Any] = {
            ...
            "normalize_hf_hidden_states": bool(normalize_hf_hidden_states),
        }
...
def _maybe_normalize_hf_hidden_state(
    model: Any,
    hidden_state: Any,
    *,
    layer_index: int,
    ...
    if not enabled or int(layer_index) >= int(total_layers):
        return hidden_state
    encoder = getattr(model, "encoder", None)
    layer_norm = getattr(encoder, "emb_layer_norm_after", None) if encoder is not None else None
    if not callable(layer_norm):
        return hidden_state
